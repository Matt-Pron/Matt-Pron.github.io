...
this.children.forEach(ch => {
    if ((isWidth ? ch.typeW : ch.typeH) === Utils.GROW.type) {
        if (isWidth) ch.computedW = ch.minW || 0;
            else ch.computedH = ch.minH || 0;
    }
});
    ...
    if (flowMatch) {
        while (remaining > 0.1 && growable.length > 0) {
            ...
            targets.forEach(ch => {
                ...
                remaining -= added;
                if (current + added >= max) growable = growable.filter(g => g !== ch);
            });
...
if (!flowMatch) {
    this.children.forEach(ch => {
        ...
        if (isWidth) ch.computedW = Math.min(ch.computedW || 0, maxChildSize);
            else ch.computedH = Math.min(ch.computedH || 0, maxChildSize);
    });
}
...
const mainAlign = isHFlow ? (this.alignment?.h ?? Utils.LEFT) : (this.alignment?.v ?? Utils.TOP);

if (mainExtra > 0) {
    if (mainAlign === Utils.CENTER) mainOffset = Math.floor(mainExtra / 2);
        else if (mainAlign === Utils.BOTTOM || mainAlign === Utils.RIGHT) mainOffset = mainExtra;
}

let currentX = (padding.l || 0) + (isHFlow ? mainOffset : 0);
let currentY = (padding.t || 0) + (!isHFlow ? mainOffset : 0);

const crossAlign = isHFlow ? (this.alignment?.v ?? Utils.TOP) : (this.alignment?.h ?? Utils.LEFT);
const availableCross = isHFlow
    ? this.computedH - padding.t - padding.b
    : this.computedW - padding.l - padding.r;

// Only clip when this container is VERTICAL + TOP (StatsViewport)
const shouldClipTop = !isHFlow && mainAlign === Utils.TOP;
...
this.children.forEach(ch => {
    ...
    if (crossExtra > 0) {
        if (crossAlign === Utils.CENTER) crossOffset = Math.floor(crossExtra / 2);
            else if (crossAlign === Utils.BOTTOM || crossAlign === Utils.RIGHT) crossOffset = crossExtra;
    }
    ...
    // Early clipping ONLY for vertical TOP containers (your stats panel)
    if (shouldClipTop) {
        if (ch.globalY + ch.computedH > this.globalY + this.computedH) {
...
// Text positioning (keeps BOTTOM for logs)
if (this.lines && this.lines.length > 0) {
    ...
    for (let i = 0; i < visibleLines; i++) {
        ...
        if (hAlign === Utils.CENTER) lineX += Math.max(0, Math.floor((availableW - line.length) / 2));
            else if (hAlign === Utils.RIGHT) lineX += Math.max(0, availableW - line.length);
...
if (this.isInteractive) {
    return { id: this.id, action: this.action, element: this };
}
return null;
    }
...
this.children.forEach(ch => {
    const chTop    = ch.globalY;
    const chBottom = ch.globalY + ch.computedH;
    const chLeft   = ch.globalX;
    const chRight  = ch.globalX + ch.computedW;
